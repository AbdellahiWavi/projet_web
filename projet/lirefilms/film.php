...
<section class="movie-detail">
  <div class="container">
    <!-- modiffier cette src -->
    <iframe width="50%" height="360" src="<?= '../'.$res->url ?>"
            ...
            $result = $req->rowCount();
              ...
              while($j){
                if($res[$i]->diffusion === 'Mieux Notes') {
                  if($res[$i] != $actuel){
                  movie_duration('../'.$res[$i]->src, $res[$i]->nom_film, $res[$i]->date_sortie,$res[$i]->vue, $res[$i]->duration, $res[$i]->note, $res[$i]->id_films,'./film.php');
                  }
                }
              $j--;
              $i++;
